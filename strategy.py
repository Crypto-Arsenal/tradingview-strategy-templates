class Strategy(StrategyBase):
    ...
    def on_tradingview_signal(self, signal, candles):
        ...
        if action == 'cancelAll' or action == 'cancel_all':
            ...
            order_quote_amount = min(self.init_available_quote_amount, available_quote_amount)
            order_price =   float(signal.get('limit')) if signal.get('limit') is not None else candles[exchange][pair][0]['close']
           
            CA.log("order_quote_amount " + str(order_quote_amount))
            CA.log("order_price " + str(order_price))

            fixed_amount = (order_quote_amount * self.percentage_of_initial_asset) / order_price
